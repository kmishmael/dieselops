...
import EngineVisualization from "./engine-visualization";
  ...
  return (
    <div className="container mx-auto p-4 max-w-7xl">
        ...
        <div className="flex flex-wrap justify-between items-center mb-4 gap-4">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm font-medium bg-muted/80 px-3 py-1.5 rounded-lg border border-border/20 hidden md:block transition-colors">
              Time: <span className="font-mono">{Math.floor(engine.time)}s</span>
            </span>
            <ThemeToggle />
          </div>
        </div>
      ...
      <Tabs defaultValue="engine" className="w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-muted rounded-lg p-4">
              ...
              <div className="text-2xl font-bold mb-2">
                {engine.emissions.co2.toFixed(1)} kg/h
              </div>
              <div className="w-full bg-muted-foreground/20 rounded-full h-2">
                <div
                  className="bg-emerald-500 h-2 rounded-full"
                  style={{ width: `${Math.min(engine.emissions.co2, 100)}%` }}
                ></div>
              </div>
            </div>
            <div className="bg-muted rounded-lg p-4">
              ...
              <div className="text-2xl font-bold mb-2">
                {engine.emissions.nox.toFixed(1)} g/kWh
              </div>
              <div className="w-full bg-muted-foreground/20 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${
                    engine.emissions.nox > 80 ? "bg-red-500" : "bg-amber-500"
                  }`}
                  style={{ width: `${Math.min(engine.emissions.nox, 100)}%` }}
                ></div>
              </div>
            </div>
            <div className="bg-muted rounded-lg p-4">
              ...
              <div className="text-2xl font-bold mb-2">
                {engine.emissions.particulates.toFixed(1)} mg/m³
              </div>
              <div className="w-full bg-muted-foreground/20 rounded-full h-2">
                <div
                  className="bg-blue-500 h-2 rounded-full"
                  style={{
                    width: `${Math.min(engine.emissions.particulates, 100)}%`,
                  }}
                ></div>
              </div>
            </div>
          ...
          <div className="mt-4 p-4 bg-muted rounded-lg">
            ...
            <p className="text-sm">
              ...
              <span className="font-bold">
                {" "}
                {Math.round(
                  (engine.emissions.co2 / 100) * 0.5 +
                    (engine.emissions.nox / 100) * 0.3 +
                    (engine.emissions.particulates / 100) * 0.2 * 100
                )}
                /100
              </span>
              .
              {engine.emissions.nox > 80
        ...
        >
          <CascadeControlPanel
            cascadeControlEnabled={engine.cascadeControlEnabled}
            setCascadeControlEnabled={engine.setCascadeControl}
            cascadeControlType={engine.cascadeControlType}
            setCascadeControlType={engine.setCascadeControlType}
            cascadeControlConfig={engine.cascadeControlConfig}
            updateCascadeSetpoint={engine.updateCascadeSetpoint}
            updateCascadeParameters={engine.updateCascadeParameters}
            cascadeParameters={engine.cascadeParameters}
            cascadeHistory={engine.cascadeHistory}
            running={engine.running}
            emergencyMode={engine.emergencyMode}
          />
        </TabsContent>
